...
test_footer_reports_usage_line_without_blocking() {
  local out
  out=$(_footer "$(_transcript claude-opus-5 2 141 120860 0)")
...
test_footer_line_pure_format() {
  local out
  out=$(claude_footer_line claude-opus-5 high 2 141 120860 0)
...
test_footer_omits_cost_for_unpriced_model() {
  local out
  out=$(_footer "$(_transcript some-other-llm 10 10 0 0)")
  local reason
  reason=$(jq -r '.systemMessage' <<<"$out")
...
test_footer_survives_quote_in_model_name() {
  local out
  out=$(_footer "$(_transcript 'weird"model' 5 5 0 0)")
  jq -e . >/dev/null 2>&1 <<<"$out"
}

test_footer_takes_the_last_assistant_message() {
  local out
  out=$(_footer "$(_transcript claude-sonnet-5 7 3 0 0)")
...
test_footer_stop_hook_active_passes_through() {
  local out
  out=$(_footer "$(_transcript claude-opus-5 2 141 120860 0)" true)
  [ "$(jq -r '.continue' <<<"$out")" = "true" ] &&
    [ "$(jq -r '.decision // "none"' <<<"$out")" = "none" ]
}
...
test_footer_missing_transcript_file_passes_through() {
  local out
  out=$(_footer "$WORK/does-not-exist.jsonl")
  [ "$(jq -r '.continue' <<<"$out")" = "true" ]
}
...
test_footer_transcript_without_assistant_passes_through() {
  ...
  local out
  out=$(_footer "$WORK/user-only.jsonl")
  [ "$(jq -r '.continue' <<<"$out")" = "true" ]
}
...
test_ctdl_agent_footer_verb() {
  local t
  t=$(_transcript claude-opus-5 2 141 120860 0)
  local out
...
test_adapter_footer_without_hook_is_silent() {
  local out
  out=$(adapter_footer nosuchagent </dev/null 2>&1)
