...
test_usage_seg_formats_pct_and_cost() {
  local out; out=$(usage_seg "$(printf 'Session\t45.5\t⟳12:00\t3.1')")
...
test_usage_seg_drops_non_numeric_cost() {
  local out; out=$(usage_seg "$(printf 'Session\t10\t⟳12:00\t;unknown')")
  ! printf '%s' "$out" | grep -qF '$'
}

test_usage_seg_omits_empty_cost() {
  local out; out=$(usage_seg "$(printf 'Today\t10\t⟳12:00\t')")
  assert_contains "$out" "Today: 10.0%" &&
  ! printf '%s' "$out" | grep -qF '$'
}

test_usage_seg_empty_without_rows_fn() {
  local out; out=$( . "$SCRIPTS/agentbar/agentbar-lib.sh"; paint_usage nosuchagent 0 )
...
test_usage_seg_puts_tokens_before_cost() {
  local out; out=$(usage_seg "$(printf 'Session\t45.5\t⟳12:00\t3.1\tT12k')")
...
test_kfmt_below_1000_shows_raw() {
  local out; out=$( . "$SCRIPTS/agentbar/agentbar-lib.sh"; kfmt_out=; kfmt 999 kfmt_out; printf '%s' "$kfmt_out" )
  [ "$out" = "999" ]
}
